...
{
	if (number != undefined && number.length > 1 && number.indexOf(".") < 0 &&(number.substring(0,1) == 'f' || number.substring(0,1) == 'F')) {
		// Numero Feminino
		number = number.substring(1,number.length);
		var ultimoNumero = number.substring(number.length-1, number.length);
		var ultimosDoisNumeros = 0;
		if (number.length > 1) {
			ultimosDoisNumeros = number.substring(number.length-2, number.length);
		}
		if ( (ultimoNumero == 1 || ultimoNumero == 2) && (ultimosDoisNumeros != 11 && ultimosDoisNumeros != 12) ) {
			//Verbalizar feminino
			numeroFeminino = true;
		}
	}
...
//			alert("tensAndOnes:::" + tensAndOnes);
		    promptsArray.push(promptBaseUrl + "units/pausa.vox");
	        	if (numeroFeminino){
	        		promptsArray.push(promptBaseUrl + "seme_men/" + tensAndOnes + "ac.vox");
		        }else{
			        if (tensAndOnes < 50) {
			...
			promptsArray.push(promptBaseUrl + "units/pausa.vox");
		        if (numeroFeminino){
		        	promptsArray.push(promptBaseUrl + "seme_men/e" + tensAndOnes + "ac.vox");
		        }else{
			        if (tensAndOnes < 50) {
				        promptsArray.push(promptBaseUrl + "seme_men/e" + tensAndOnes + ".vox");
					} else {
						promptsArray.push(promptBaseUrl + "seme_men/e" + tensAndOnes + ".vox");
			        }
			}	
...
function digitoadigito(string)
{
    ...
    for (i = 0; i < string.length; i++) {
		...
		if (i >= string.length - 1) {
			promptsArray.push(promptBaseUrl + "numeros/f_" + ch + ".vox");
		} else {
			promptsArray.push(promptBaseUrl + "numeros/m_" + ch + ".vox");
		}
	}
    return promptsArray;
}
